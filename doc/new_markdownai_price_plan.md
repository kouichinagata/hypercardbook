...
* **一般ユーザー / 協力者**:
  * Standard / Pro / Enterpriseボタンを押すと、`Enter activation code` の入力欄と `Apply` ボタンが表示されます。
  * 有効なプロモーションコードを入力すると、コードに設定された期間だけ対象プランが有効になります。
  * 期限が切れると、恒久プラン（通常はFree）へ自動的に戻ります。
* **管理者（永田様）**:
  * コードなしで各プランへ切り替え、テストできます。
  * 配布用コードは、画面や環境変数ではなく本番SupabaseのプロモーションコードDBをアリスが管理します。
